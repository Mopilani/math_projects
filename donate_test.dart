...
dynamic add3DShape(
    ...
    num z_rotation_on_y = 180,
    num yz_rotation_value = 180,
  ...
  for (var i = 0; i < 60; i++) {
    ...
    if (def3 != null) {
      ...
      z = math.cos(i) * math.sin(rotationValue);
    }
    var rotationValue = yz_rotation_value *
        .0088; // this ".00875 ~ " must be for to know the shape hade rotated 180 DEG around x axis
    yz = math.sin(i) * math.cos(rotationValue);
    ...
    var yzmhd = yz * dimeterHalf;

    /// Height half
    var heightHalf = screenHeight * fromHeight;
    var realY = heightHalf + ymhd;
    var realYz = heightHalf + yzmhd;
    // purpose pixel
    var py = screenWidth * realY.round();
    var pyz = screenWidth * realYz.round();
    ...
    int purposeIndex = (py + px).round();
    int seconderyIndex = (pyz + pz).round();
    // int closuerIndex = (purposeIndex + seconderyIndex).round();
    ...
    try {
      ...
      // if (rotated_on_x) {
      //   framPixels[closuerIndex] = _coloredString('!');
      // } else {
      //   framPixels[closuerIndex] = ' ';
      // }
...
void main(List<String> args) async {
  ...
  try {
    ...
    var z_rotation_value = 180;
    var yz_rotation_value = 180;
    var y_clamping = true;
    var x_clamping = true;
    var z_clamping = true;
    var yz_clamping = true;
    for (var i = 0; i < (loops ??= 4040); i++) {
      /// the 2 milli seconds delay give us a 500 FPS for development
      await Future.delayed(Duration(microseconds: 1), () { 
        // X rotation value sumation
        if (x_rotation_value < 360 && x_clamping) {
          x_rotation_value++;
        } else if (x_rotation_value <= 360) {
          x_clamping = false;
        ...
        if (y_rotation_value < 360 && y_clamping) {
          y_rotation_value++;
        } else if (y_rotation_value <= 360) {
          y_clamping = false;
        ...
        // }
        // Y rotation value sumation
        if (yz_rotation_value > 0 && yz_clamping) {
          yz_rotation_value--;
        } else if (yz_rotation_value >= 0) {
          yz_clamping = false;
          if (yz_rotation_value == 360) {
            yz_clamping = true;
          } else {
            yz_rotation_value++;
          }
        ...
        // Z rotation value sumation
        if (z_rotation_value < 360 && z_clamping) {
          z_rotation_value++;
        } else if (z_rotation_value <= 360) {
          z_clamping = false;
          if (z_rotation_value == 0) {
            z_clamping = true;
          } else {
            z_rotation_value--;
          }
        ...
        // add3DShape(
        //   fromHeight:
        //       y_clamping ? rotat_on_yw() : rotat_on_xw().riseToThePower(4),
        //   fromWidth: y_clamping ? rotat_on_xw() : rotat_on_yw().riseToThePower(3),
        //   dimeterHalf: dimeter ??= 2,
        //   char: _coloredString('0', colorState: Colors.standerd),
        //   screenWidth: screenWidth,
        //   framPixels: framPixels,
        //   def1: xx,
        //   def2: yy,
        //   z_rotation_on_y: z_rotation_value,
        //   screenHeight: screenHeight,
        // );

        // add3DShape(
        //   fromHeight:
        //       y_clamping ? rotat_on_xw() : rotat_on_yw().riseToThePower(3),
        ...
        //   fromWidth: y_clamping ? rotat_on_yw() : rotat_on_xw().riseToThePower(4),
        //   dimeterHalf: 2,
        //   char: _coloredString('0', colorState: Colors.standerd),
        //   screenWidth: screenWidth,
        //   framPixels: framPixels,
        ...
        D3Ball(
          ...
          yz_rotation_value: yz_rotation_value,
        );
        ...
        //   rotated_on_y: y_clamping,
        //   screenHeight: screenHeight,
        //   z_rotation_on_y: z_rotation_value - 180,
        //   x_rotation_on_y: x_rotation_value  - 180,
        //   y_rotation_on_x: y_rotation_value  - 180,
        //   yz_rotation_value: yz_rotation_value  - 180,
        // );
...
D3Ball(
    ...
    num fromWidth = .5,
    num fromHeight = .5,
    num x_rotation_on_y = 180,
    num y_rotation_on_x = 90,
    num z_rotation_on_y = 0,
    num yz_rotation_value = 0,
    bool rotated_on_x = false,
    bool rotated_on_y = false}) {
  add3DShape(
    dimeterHalf: 14,
    char: _coloredString(',', colorNumber: 34),
    screenWidth: screenWidth,
    screenHeight: screenHeight,
    framPixels: framPixels,
    def1: def1,
    ...
    x_rotation_on_y: x_rotation_on_y,
    y_rotation_on_x: y_rotation_on_x,
    yz_rotation_value: yz_rotation_value,
  );
  // add3DShape(
  //   dimeterHalf: 15,
  //   char: _coloredString(',', colorNumber: 33),
  //   screenWidth: screenWidth,
  //   framPixels: framPixels,
  //   def1: def1,
  //   def2: def2,
  ...
  //   rotated_on_x: rotated_on_x,
  //   screenHeight: screenHeight,
  //   z_rotation_on_y: z_rotation_on_y - 0,
  //   x_rotation_on_y: x_rotation_on_y - 180,
  //   y_rotation_on_x: y_rotation_on_x - 0,
  // );

  // add3DShape(
  //   dimeterHalf: 14,
  //   char: _coloredString('2', colorNumber: 32),
  //   screenWidth: screenWidth,
  //   framPixels: framPixels,
  //   def1: def1,
  //   def2: def2,
  ...
  //   rotated_on_x: rotated_on_x,
  //   screenHeight: screenHeight,
  //   z_rotation_on_y: z_rotation_on_y - 180,
  //   x_rotation_on_y: x_rotation_on_y + 180,
  //   y_rotation_on_x: y_rotation_on_x + 180,
  // );
  // add3DShape(
  //   dimeterHalf: 15,
  //   char: _coloredString('1', colorNumber: 31),
  //   screenWidth: screenWidth,
  //   framPixels: framPixels,
  //   def1: def1,
  //   def2: def2,
